//
//  VTPhotoCollectionViewController.swift
//  Virtual Tourist
//
...
class VTPhotoAlbumViewController: UIViewController, MKMapViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource, NSFetchedResultsControllerDelegate {
    
    let NoImageMessage = "No Images"
    ...
    @IBAction func newController(sender: AnyObject) {
        DataModel.searchPhotos(pinAnnotation.createdAt!, inNewCollection: true)
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        addObservers()
        DataModel.searchPhotos(pinAnnotation.createdAt!, inNewCollection: false)
    }
    ...
    func addObservers() {
        defaultCenter.addObserver(self, selector: #selector(VTPhotoAlbumViewController.searchPhotosStarted), name: DataModel.NotificationNames.SearchPhotosStarted, object: nil)
        defaultCenter.addObserver(self, selector: #selector(VTPhotoAlbumViewController.searchPhotosPending), name: DataModel.NotificationNames.SearchPhotosPending, object: nil)
        defaultCenter.addObserver(self, selector: #selector(VTPhotoAlbumViewController.searchPhotosCompleted), name: DataModel.NotificationNames.SearchPhotosCompleted, object: nil)
        defaultCenter.addObserver(self, selector: #selector(VTPhotoAlbumViewController.photoDownloadCompleted), name: DataModel.NotificationNames.PhotoDownloadCompleted, object: nil)
        defaultCenter.addObserver(self, selector: #selector(VTPhotoAlbumViewController.allPhotoDownloadsCompleted), name: DataModel.NotificationNames.AllPhotoDownloadsCompleted, object: nil)
    }
    ...
    func searchPhotosCompleted(){
        dispatch_async(dispatch_get_main_queue()){
            ...
            self.toggleMessageLabel(count == 0 ? self.NoImageMessage : nil)
        }
    }
    ...
    func mapView(mapView: MKMapView, viewForAnnotation annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        if pinView == nil {
            // Create a new pin view and intialize
            pinView = MKPinAnnotationView(annotation: annotation, reuseIdentifier: reuseId)
            pinView!.pinTintColor = MKPinAnnotationView.redPinColor()
            pinView!.draggable = true
            pinView!.animatesDrop = true
        } else {
            // Reuse existing pin view and set the annotation
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        let photoCollectionViewCell = collectionView.dequeueReusableCellWithReuseIdentifier("VTPhotoCollectionViewCell", forIndexPath: indexPath) as! VTPhotoCollectionViewCell
    ...
    func configureCell(cell: VTPhotoCollectionViewCell, photo: Photo) {
